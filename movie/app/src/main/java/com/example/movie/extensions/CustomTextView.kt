...
class CustomTextView @kotlin.jvm.JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : AppCompatTextView(context, attrs, defStyleAttr) {

    private var showingLine = 4

    private var showMore = "read more"
    private var showLess = "hide"
    ...
    private fun showMoreButton() {
        ...
        do {
            ...
            newText += "$threeDot ${showMore.lowercase()}"
            setText(newText)
            specialSpace++
    ...
    private fun showLessButton() {
        val text = "$text ${showLess.lowercase()}"
        val spannableString = SpannableString(text)
        spannableString.setSpan(
            object : ClickableSpan() {
